...
async def main():
    ...
    for i, item in enumerate(data_source):
        ...
        output_path = os.path.join(output_dir, f"question_{i}.mp3")
        print(f"生成问题: {question}")
        success = await generate_audio(question, output_path)
        if success:
            audio_mapping[question] = f"voice/game_qa/question_{i}.mp3"
    ...
    for i, item in enumerate(data_source):
        ...
        if success:
            audio_mapping[answer] = f"voice/game_qa/answer_{i}.mp3"
    ...
    print(f"\n音频生成完成！")
    print(f"生成了 {len(audio_mapping)} 个音频文件")
    print(f"映射关系已保存到: {mapping_path}")
